...
- **`KelpieCore`** — pure logic, no I/O, no AppKit, no `Foundation` socket or
  ...
  `NotificationPolicy`, `BlockedReminder`, `BlockedEmphasis`. Every behavioral
  rule Kelpie depends on — bootstrap is silent, a live transition into
  `blocked` notifies exactly once, no ordinary update re-fires it, a pane left
  blocked is reminded about on a widening 1/5/15-minute schedule (and one
  blocked before launch never is), a blocked count left unanswered starts
  inverting after a minute and inverts faster after five, and a burst of change
  signals coalesces into one refresh without starving — lives here and is
